...
Tự động thêm tên file làm tiêu đề để phân biệt nội dung
"""
...
def merge_txt_files(input_folder, output_file='merged_output.txt', separator='\n\n', add_file_header=True):
    ...
    Args:
        ...
        separator (str): Ký tự phân cách giữa các file (mặc định: 2 xuống dòng)
        add_file_header (bool): Thêm tên file làm tiêu đề (mặc định: True)
    """
    ...
    for txt_file in txt_files:
        ...
        try:
            ...
            # Tạo header với tên file nếu cần
            if add_file_header:
    ...
    if merged_content:
        try:
            ...
            print(f"   - Tiêu đề file: {'Có' if add_file_header else 'Không'}")
...
def main():
    ...
    # Hỏi có thêm header không
    add_header = input("📋 Thêm tên file làm tiêu đề? (Y/n, Enter = Yes): ").strip().lower()
    add_file_header = add_header != 'n'
    
    print()
    
    # Gộp file
    merge_txt_files(input_folder, output_file, separator='\n\n', add_file_header=add_file_header)
